...
let () =
  ...
  let entropy =
    "\x46\xe6\x23\x70\xa1\x38\xa1\x82\xa4\x98\xb8\xe2\x88\x5b\xc0\x32\x37\x9d\xdf\x38"
  in
  ...
  | Ok root ->
      say
        ("icarus master  = "
        ^ hex (String.sub (K.Xprv.to_bytes root) 0 16)
        ^ "...\n");
      ...
      let body =
        {
          ...
          B.outputs =
            [
              B.Output.make addr
                (T.Value.of_coin (Result.get_ok (T.Coin.of_lovelace 2000000L)));
            ];
        }
      in
      ...
      say
        ("signature ok   = "
        ^ string_of_bool
            (K.Xpub.verify pub ~signature:sg
               (T.Hash.Tx_id.to_bytes (B.id body)))
        ^ "\n");
