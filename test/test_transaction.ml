...
let sample () =
  { B.empty with
    ...
    B.outputs = [ B.Output.make (addr ()) (T.Value.of_coin (get (Result.map_error (fun _ -> "coin") (T.Coin.of_lovelace 2_000_000L)))) ];
    B.fee = get (Result.map_error (fun _ -> "coin") (T.Coin.of_lovelace 170_000L)) }

let round_trip () =
  let b = sample () in
  let enc = B.to_cbor b in
  let d = get (B.of_cbor enc) in
  Alcotest.(check string) "decode then encode is the identity" (hex_of enc) (hex_of (B.to_cbor d));
  Alcotest.(check string) "the id is stable across the round trip"
    (T.Hash.Tx_id.to_hex (B.id b)) (T.Hash.Tx_id.to_hex (B.id d));
  Alcotest.(check bool) "a built body has no preserved bytes" false (B.is_verbatim b);
...
let non_canonical_input_preserved () =
  ...
  in
  Alcotest.(check bool) "the two spellings really do differ" false
    (String.equal ours respelt);
  let d = get (B.of_cbor respelt) in
  Alcotest.(check string) "the sender's bytes are preserved verbatim"
    (hex_of respelt) (hex_of (B.to_cbor d));
  (* And therefore the id is the sender's id, not ours. *)
  let expected = hex_of (T.blake2b256 respelt) in
  Alcotest.(check string) "the id is over the bytes as received" expected
    (T.Hash.Tx_id.to_hex (B.id d));
  Alcotest.(check bool) "which is not the id we would have computed from fields"
    false
...
let carried_fields_survive () =
  ...
  let with_gov =
    ...
    | C.Map fields ->
        C.encode
          (C.Map (fields @ [ (C.Uint 19L, C.Map [ (C.Bytes "voter", C.Uint 1L) ]);
                             (C.Uint 22L, C.Uint 500L) ]))
    | _ -> Alcotest.fail "body should be a map"
  in
  let d = get (B.of_cbor with_gov) in
  Alcotest.(check int) "both uninterpreted fields are carried" 2 (List.length d.B.carried);
  Alcotest.(check bool) "voting procedures among them" true (List.mem_assoc 19 d.B.carried);
  Alcotest.(check bool) "donation among them" true (List.mem_assoc 22 d.B.carried);
  Alcotest.(check string) "and the body still re-encodes byte for byte"
    (hex_of with_gov) (hex_of (B.to_cbor d))
...
let update_field_refused () =
  ...
  let with_update =
    ...
    | _ -> Alcotest.fail "body should be a map"
  in
  Alcotest.(check bool) "key 6 is refused, not ignored" true
...
let multi_asset_output () =
  ...
  let v = T.Value.make (get (Result.map_error (fun _ -> "coin") (T.Coin.of_lovelace 3_000_000L))) assets in
  let b = { (sample ()) with B.outputs = [ B.Output.make (addr ()) v ] } in
  let d = get (B.of_cbor (B.to_cbor b)) in
  match d.B.outputs with
  | [ o ] ->
      Alcotest.(check bool) "the bundle survives" true
        (T.Value.equal o.B.Output.value v);
      Alcotest.(check bool) "and is not ada-only" false (T.Value.is_ada_only o.B.Output.value)
...
let ada_only_is_a_bare_coin () =
  ...
  | C.Map fields -> (
      ...
      | C.Array [ C.Map fs ] -> (
          ...
          | C.Uint _ -> ()
          | v -> Alcotest.failf "ada-only value should be a bare uint, got %s" (hex_of (C.encode v)))
...
let unsigned () =
  { Tx.body = sample (); Tx.witness_set = W.empty; Tx.is_valid = true;
    Tx.auxiliary_data = None; Tx.raw = None }
...
let id_is_stable_under_signing () =
  ...
  Alcotest.(check string) "signing does not move the id" before
    (T.Hash.Tx_id.to_hex (Tx.id signed));
  Alcotest.(check int) "one witness attached" 1 (List.length (Tx.witnesses signed));
  let twice = Tx.sign signed (key ()) in
  Alcotest.(check int) "signing twice attaches one witness" 1
...
let signs_the_id_not_the_body () =
  let t = unsigned () in
  Alcotest.(check int) "the payload is 32 bytes" 32
    (String.length (Tx.signing_payload t));
  Alcotest.(check string) "and it is the transaction id"
    (T.Hash.Tx_id.to_hex (Tx.id t)) (hex_of (Tx.signing_payload t));
  let k = key () in
  let over_body = K.Xprv.sign k (B.to_cbor t.Tx.body) in
  let vkey = K.Xpub.raw (K.Xprv.public k) in
  Alcotest.(check bool) "a signature over the body is refused" true
...
let external_signer () =
  ...
  Alcotest.(check bool) "and it verifies" true
    (W.Vkey.verify (List.hd (Tx.witnesses signed)) (Tx.id signed));
  Alcotest.(check string) "witness key hash matches the public key's"
  ...
  Alcotest.(check bool) "mismatched key and signature refused" true
...
let signed_transaction_round_trips () =
  ...
  Alcotest.(check string) "a signed transaction round-trips" (hex_of enc)
    (hex_of (Tx.to_cbor d));
  Alcotest.(check string) "with the same id"
    (T.Hash.Tx_id.to_hex (Tx.id signed)) (T.Hash.Tx_id.to_hex (Tx.id d));
  Alcotest.(check int) "and the same witnesses" 1 (List.length (Tx.witnesses d));
  Alcotest.(check bool) "which still verify" true
  ...
  Alcotest.run "cardano-transaction"
    [ ("body",
       [ Alcotest.test_case "round trip" `Quick round_trip;
         ...
         Alcotest.test_case "ada-only value shape" `Quick ada_only_is_a_bare_coin ]);
      ("byte preservation",
       [ Alcotest.test_case "non-canonical set spelling" `Quick non_canonical_input_preserved;
         Alcotest.test_case "uninterpreted fields survive" `Quick carried_fields_survive ]);
      ("era",
       [ Alcotest.test_case "conway rejects the update field" `Quick update_field_refused ]);
      ("signing",
       [ Alcotest.test_case "id is stable under signing" `Quick id_is_stable_under_signing;
         Alcotest.test_case "signs the id, not the body" `Quick signs_the_id_not_the_body;
         Alcotest.test_case "external signer" `Quick external_signer;
         Alcotest.test_case "signed round trip" `Quick signed_transaction_round_trips ]) ]
