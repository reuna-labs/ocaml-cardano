...
let round_trip () =
  List.iter
    (fun (kind, s) ->
      ...
      Alcotest.(check string) (Printf.sprintf "mainnet type-%02d" kind) s
        (get (A.to_bech32 a));
      Alcotest.(check bool) (Printf.sprintf "mainnet type-%02d is mainnet" kind)
        true (N.equal (A.network a) N.mainnet))
    mainnet;
  List.iter
    (fun (kind, s) ->
      let a = get (A.of_bech32 s) in
      Alcotest.(check string) (Printf.sprintf "testnet type-%02d" kind) s
        (get (A.to_bech32 a));
      Alcotest.(check bool) (Printf.sprintf "testnet type-%02d is testnet" kind)
        false (N.equal (A.network a) N.mainnet))
    testnet
...
let shapes () =
  ...
  (match a 4 with A.Pointer _ -> () | _ -> Alcotest.fail "type-04 should be pointer");
  (match a 6 with A.Enterprise _ -> () | _ -> Alcotest.fail "type-06 should be enterprise");
  (match a 14 with A.Reward _ -> () | _ -> Alcotest.fail "type-14 should be reward");
...
let pointers () =
  ...
  match a with
  | A.Pointer { pointer; _ } ->
      Alcotest.(check string) "pointer round-trips" (Format.asprintf "%a" A.Pointer.pp pointer)
        (Format.asprintf "%a" A.Pointer.pp pointer);
      let re = get (A.of_bytes (A.to_bytes a)) in
      Alcotest.(check bool) "pointer bytes round-trip" true
...
let rejections () =
  ...
  (* A single altered character must fail the checksum rather than decode to a
     different address. *)
  let flip s i = String.mapi (fun j c -> if j = i then (if c = 'q' then 'p' else 'q') else c) s in
  Alcotest.(check bool) "single-character mutation is caught" true
  ...
  let mislabelled =
    get (Result.map_error (fun m -> `Bech32 m)
           (Cardano_address.Bech32.encode_bytes Cardano_address.Bech32.Bech32
              ~hrp:"addr" stake_payload))
  in
  Alcotest.(check bool) "hrp/payload mismatch is refused" true
  ...
  in
  Alcotest.(check bool) "network mismatch is refused" true
    (match A.of_bech32 wrong_net with Error _ -> true | Ok _ -> false);
  Alcotest.(check bool) "empty input refused" true
    (match A.of_bytes "" with Error _ -> true | Ok _ -> false);
  Alcotest.(check bool) "short base address refused" true
    (match A.of_bytes (String.make 40 '\000') with Error _ -> true | Ok _ -> false);
  ...
  Alcotest.(check bool) "reserved header refused" true
    (match A.of_bytes (String.make 1 '\160' ^ String.make 28 '\000') with
     | Error (`Unknown_header _) -> true | _ -> false)
...
let byron () =
  ...
  | Ok (A.Byron b) ->
      ...
      Alcotest.(check bool) "has no bech32 form" true
        (match A.to_bech32 (A.Byron b) with
         | Error `Byron_unsupported -> true | _ -> false)
