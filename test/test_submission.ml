...
let prepared ?(ttl = Some 1000L) ?(has_scripts = false) ?(id = '\001') () =
  { S.cbor = "\132\160"; S.tx_id = tx_id id; S.ttl; S.inputs = [ input '\002' ];
    S.has_scripts }
...
let outcome st =
  ...
  | a ->
      Alcotest.failf "expected a finished machine, got %s"
        (match a with
         | S.Fetch_tip -> "Fetch_tip" | S.Evaluate _ -> "Evaluate"
         | S.Rebuild _ -> "Rebuild" | S.Submit _ -> "Submit"
         | S.Check_inputs _ -> "Check_inputs" | S.Wait -> "Wait"
...
let happy_path () =
  ...
  Alcotest.(check bool) "starts by asking for the tip" true (S.action st = S.Fetch_tip);
  let st = step st (S.Tip { slot = 10L; height = 100L }) in
  (* No scripts, so no evaluation round. *)
  Alcotest.(check bool) "goes straight to submitting" true
    (match S.action st with S.Submit _ -> true | _ -> false);
  let st = step st (S.Submitted (tx_id '\001')) in
  Alcotest.(check bool) "then watches the inputs" true
  ...
  Alcotest.(check bool) "an unspent input means keep waiting" true
    (match S.action st with S.Check_inputs _ -> true | _ -> false);
  (* Gone from the UTXO set: it is in a block. *)
  let st = step st (S.Unspent_inputs []) in
  Alcotest.(check bool) "then counts depth against the tip" true
    (S.action st = S.Fetch_tip);
  let st = step st (S.Tip { slot = 12L; height = 100L }) in
  Alcotest.(check bool) "one block deep is not two" true
  ...
  | S.Accepted { depth; tx_id = id } ->
      ...
      Alcotest.(check string) "for the transaction we sent"
        (T.Hash.Tx_id.to_hex (tx_id '\001')) (T.Hash.Tx_id.to_hex id)
...
let expired_validity_interval () =
  ...
  Alcotest.(check bool) "the last valid slot is still valid" true
    (match S.action st with S.Submit _ -> true | _ -> false);
  ...
  Alcotest.(check bool) "an unbounded transaction never expires" true
    (match S.action st with S.Submit _ -> true | _ -> false)
...
let script_evaluation_rebuilds () =
  ...
  let st = step st (S.Tip { slot = 10L; height = 100L }) in
  Alcotest.(check bool) "a scripted transaction is evaluated first" true
  ...
  (match S.action st with
   | S.Rebuild bs ->
       Alcotest.(check int) "and hands the budgets back to be applied" 1 (List.length bs)
  ...
  Alcotest.(check string) "the machine follows the new id"
    (T.Hash.Tx_id.to_hex (tx_id '\007')) (T.Hash.Tx_id.to_hex (S.tx_id st));
  ...
  Alcotest.(check bool) "validity is re-checked against the new transaction" true
    (S.action st = S.Fetch_tip);
  ...
  Alcotest.(check bool) "and re-evaluated rather than submitted on trust" true
    (match S.action st with S.Evaluate _ -> true | _ -> false)
...
let evaluation_converges () =
  ...
  Alcotest.(check bool) "the first answer triggers a rebuild" true
    (match S.action !st with S.Rebuild _ -> true | _ -> false);
  st := step !st (S.Rebuilt (prepared ~has_scripts:true ~id:'\007' ()));
  st := step !st (S.Tip { slot = 1L; height = 1L });
  Alcotest.(check bool) "and the rebuilt transaction is evaluated again" true
    (match S.action !st with S.Evaluate _ -> true | _ -> false);
  st := step !st (S.Evaluated [ budget "spend:0" 1000L 2000L ]);
  Alcotest.(check bool) "the same answer twice means it is settled" true
    (match S.action !st with S.Submit _ -> true | _ -> false);
...
let rebuild_loop_is_bounded () =
  let st = ref (S.start (cfg ~max_rebuilds:2 ()) (prepared ~has_scripts:true ())) in
  let n = ref 0 in
  let rec spin guard =
...
let node_disagreeing_about_the_id () =
  ...
  | S.Failed (S.Id_disagreement { expected; reported }) ->
      Alcotest.(check string) "reports what we sent"
        (T.Hash.Tx_id.to_hex (tx_id '\001')) (T.Hash.Tx_id.to_hex expected);
      Alcotest.(check string) "and what the node claimed"
        (T.Hash.Tx_id.to_hex (tx_id '\099')) (T.Hash.Tx_id.to_hex reported)
...
let rollback_resubmits_then_gives_up () =
  ...
  Alcotest.(check bool) "a rollback resubmits" true
    (match S.action !st with S.Submit _ -> true | _ -> false);
  st := step !st (S.Submitted (tx_id '\001'));
  st := step !st (S.Unspent_inputs []);
  st := step !st (S.Unspent_inputs [ input '\002' ]);
  ...
  | o ->
      Alcotest.failf "expected a give-up, got %s"
        (match o with S.Accepted _ -> "acceptance" | S.Failed f -> Format.asprintf "%a" S.pp_failure f)

(* A ledger rejection is final; a dropped connection is not. *)
let errors_are_classified () =
  let st = S.start (cfg ()) (prepared ()) in
  let st' = step st (S.Rpc_error (E.Rpc { code = 3005; message = "no"; data = Some (Yojson.Safe.from_string {|"why"|}) })) in
  (match outcome st' with
  ...
  Alcotest.(check bool) "a transport failure is retried, not fatal" true
    (match S.action st'' with S.Finished _ -> false | _ -> true)
  ...
  let rec spin n =
    if n > 10 then Alcotest.fail "polling was not bounded"
      ...
      | S.Finished (S.Failed (S.Confirmation_timeout k)) ->
          Alcotest.(check bool) "gives up after the configured number" true (k >= 3)
      | _ ->
          st := step !st (S.Unspent_inputs [ input '\002' ]);
          spin (n + 1)
  in
...
let config_refuses_nonsense () =
  Alcotest.(check bool) "zero confirmations is not a policy" true
    (match S.config ~confirmations:0 () with Error _ -> true | Ok _ -> false);
  Alcotest.(check bool) "nor is zero polls" true
    (match S.config ~max_confirmation_polls:0 () with Error _ -> true | Ok _ -> false);
  Alcotest.(check bool) "no rebuilds at all is a valid choice" true
    (match S.config ~max_rebuilds:0 () with Ok _ -> true | Error _ -> false)

let () =
  Alcotest.run "cardano-submission"
    [ ("happy path", [ Alcotest.test_case "submit and confirm" `Quick happy_path ]);
      ("validity",
       [ Alcotest.test_case "expired interval" `Quick expired_validity_interval ]);
      ("scripts",
       [ Alcotest.test_case "evaluation triggers a rebuild" `Quick script_evaluation_rebuilds;
         Alcotest.test_case "evaluation converges" `Quick evaluation_converges;
         Alcotest.test_case "the rebuild loop is bounded" `Quick rebuild_loop_is_bounded ]);
      ("chain conditions",
       [ Alcotest.test_case "node disagrees about the id" `Quick node_disagreeing_about_the_id;
