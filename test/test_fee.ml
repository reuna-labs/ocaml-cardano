...
let ledger_worked_example () =
  Alcotest.(check int64)
    "arithmetic behind the stated floor" 857_690L
    (Int64.mul (Int64.add 160L 39L) 4_310L);
  let one =
    T.Value.of_coin
      (get (Result.map_error (fun _ -> "c") (T.Coin.of_lovelace 1L)))
  in
  Alcotest.(check int64)
    "an enterprise output settles at the stated floor" 857_690L
    (lov (get (F.settled_min_utxo pp (B.Output.make (addr ()) one))));
  Alcotest.(check int64)
    "a base output settles at the stated figure" 978_370L
...
let min_utxo_shape () =
  ...
  Alcotest.(check bool)
    "a one-credential output is close to the stated floor" true
    (Int64.compare (lov need) 900_000L < 0);
  Alcotest.(check bool)
    "an output holding one lovelace does not meet it" false
...
let reference_script_tiers () =
  ...
  Alcotest.(check int64)
    "one tier plus a byte"
    (Int64.add (Int64.mul 25_600L 15L) 18L)
    (f 25_601);
  (* Two full tiers: 25600*15 + 25600*18. *)
  Alcotest.(check int64)
    "two full tiers"
    (Int64.add (Int64.mul 25_600L 15L) (Int64.mul 25_600L 18L))
    (f 51_200);
  ...
  let half =
    {
      pp with
      P.min_fee_ref_script_coins_per_byte = get (T.Rational.of_ratio 15L 2L);
    }
  in
  Alcotest.(check int64)
    "a fractional charge rounds down" 7L
...
let execution_units_round_up () =
  ...
  Alcotest.(check int64)
    "a fraction of a lovelace rounds up to one" 1L (f 1L 0L);
  ...
  Alcotest.(check bool)
    "the maximum budget computes without overflow" true
    (Int64.compare full 0L > 0);
  Alcotest.(check int64)
    "and equals ceil(577*14e6/1e4 + 721*1e10/1e7)"
    (Int64.add
...
let base_fee () =
  ...
  Alcotest.(check int64)
    "an empty transaction is the constant term" 155_381L
    (lov (get (F.min_fee pp ~tx_size:0 ())));
  Alcotest.(check int64)
    "and each byte adds minFeeA"
  ...
  Alcotest.(check string)
    "which reads as ada" "0.168581"
    (T.Coin.to_ada_string simple);
  Alcotest.(check bool)
    "a negative size is refused" true
...
let collateral () =
  let c f =
    lov
      (get
         (F.required_collateral pp
            (get (Result.map_error (fun _ -> "c") (T.Coin.of_lovelace f)))))
  in
...
let rational_is_exact () =
  ...
  Alcotest.(check bool)
    "and three thirds are exactly one" true (R.equal one R.one);
  ...
  Alcotest.(check bool)
    "overflow is refused" true
    (match R.mul_int64 (get (R.of_int64 Int64.max_int)) 2L with
    | Error _ -> true
    | Ok _ -> false);
  Alcotest.(check bool)
    "a negative denominator is refused" true
...
let () =
  Alcotest.run "cardano-fee"
    [
      ( "minimum utxo",
        [
          Alcotest.test_case "ledger worked example" `Quick
            ledger_worked_example;
          Alcotest.test_case "formula shape" `Quick min_utxo_shape;
        ] );
      ( "fees",
        [
          Alcotest.test_case "base fee" `Quick base_fee;
          Alcotest.test_case "reference script tiers" `Quick
            reference_script_tiers;
          Alcotest.test_case "execution units round up" `Quick
            execution_units_round_up;
          Alcotest.test_case "collateral" `Quick collateral;
        ] );
      ( "arithmetic",
        [ Alcotest.test_case "rationals are exact" `Quick rational_is_exact ] );
    ]
