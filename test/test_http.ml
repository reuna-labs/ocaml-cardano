...
let both ?(name = "") input expected =
  List.iter
    (fun (label, chunk) ->
      ...
      | `Done body -> Alcotest.(check string) (name ^ " " ^ label) expected body
...
let status_is_reported () =
  let st = H.start () in
  match
    H.feed st "HTTP/1.1 503 Service Unavailable\r\nContent-Length: 0\r\n\r\n"
  with
...
let close_delimited () =
  ...
  | H.Need_more st -> (
      ...
      | H.Done body ->
          Alcotest.(check string) "body ends at close" "partial" body
...
let limits_are_enforced () =
  ...
  let long_headers =
    "HTTP/1.1 200 OK\r\n"
    ^ String.concat ""
        (List.init 20 (fun i -> Printf.sprintf "X-Pad-%d: aaaaaaaa\r\n" i))
  in
...
let () =
  Alcotest.run "cardano-http"
    [
      ...
      ( "request",
        [ Alcotest.test_case "well formed" `Quick request_is_well_formed ] );
    ]
