...
let constructors () =
  ...
  Alcotest.(check string)
    "Constr 0 with no fields" "d87980"
  ...
  Alcotest.(check string)
    "Constr 7 starts the second range" "d9050080"
  ...
  Alcotest.(check bool)
    "a large alternative uses tag 102" true
    (String.length big > 2 && hex_of (String.sub big 0 2) = "d866");
  List.iter
    (fun a ->
      round_trip
        (Printf.sprintf "Constr %d" a)
...
let long_bytes_are_chunked () =
  ...
  Alcotest.(check string)
    "64 bytes stays definite" "5840"
    (hex_of (String.sub e 0 2));
  let long = String.make 65 'a' in
  let e = P.encode (P.Bytes long) in
  Alcotest.(check string)
    "65 bytes becomes indefinite" "5f"
    (hex_of (String.sub e 0 1));
  Alcotest.(check string)
    "with a full first chunk" "5840"
    (hex_of (String.sub e 1 2));
  Alcotest.(check string)
    "then a one-byte chunk" "4161"
  ...
  Alcotest.(check bool)
    "a chunked child survives nesting" true
...
let maps_keep_their_order () =
  ...
  let e = P.encode d in
  Alcotest.(check string)
    "written in the order given" "a2020001 00"
...
let language_views_match_the_cddl () =
  ...
  Alcotest.(check string)
    "with V1's doubly-encoded key, 4100" "4100" (String.sub h 2 4);
  Alcotest.(check string)
    "and an indefinite list inside a byte string" "58a89f" (String.sub h 6 6);
  Alcotest.(check string)
    "terminated by a break" "ff"
...
let language_views_sort_by_length () =
  let views =
    S.language_views [ (S.Plutus_v1, [ 0L ]); (S.Plutus_v2, [ 0L ]) ]
  in
  let h = hex_of views in
  ...
  Alcotest.(check bool) "and the longer one follows" true (String.length h > 8)

let script_data_hash_shapes () =
  let models = [ (S.Plutus_v2, [ 0L ]) ] in
  let datum = P.encode (P.of_int 1) in
  let h ~redeemers ~datums =
    T.Hash.Script_data_hash.to_hex
      (S.script_data_hash ~redeemers ~datums ~cost_models:models)
  in
  (* Datums but no redeemers is A0 || datums || A0 -- the language views are
     not included at all in that case. *)
  let expected = hex_of (T.blake2b256 ("\xa0" ^ "\x81" ^ datum ^ "\xa0")) in
  Alcotest.(check string)
    "datums without redeemers" expected
  ...
  Alcotest.(check string)
    "redeemers without datums" expected
    ...
    [
      ( "plutus data",
        [
          ...
          Alcotest.test_case "long bytes are chunked" `Quick
            long_bytes_are_chunked;
      ...
      ( "scripts",
        [
          Alcotest.test_case "language-tagged hashes" `Quick
            script_hashes_are_language_tagged;
        ] );
      ( "script data hash",
        [
          Alcotest.test_case "language views match the CDDL" `Quick
            language_views_match_the_cddl;
          Alcotest.test_case "views sort by length" `Quick
            language_views_sort_by_length;
