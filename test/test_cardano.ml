...
(* Blake2b mixes the digest length into its parameter block, so the 224-bit
   ...
   nothing. *)
let blake2b_widths () =
  Alcotest.(check string) "blake2b-256 of empty"
    "0e5751c026e543b2e8ab2eb06099daa1d1e5df47778f7787faab45cdf12fe3a8"
    (hex_of (T.blake2b256 ""));
  Alcotest.(check string) "blake2b-224 of empty"
    "836cc68931c2e4e3e838602eca1902591d216837bafddfe6f0c8cb07"
    (hex_of (T.blake2b224 ""));
  Alcotest.(check string) "blake2b-256 of abc"
    "bddd813c634239723171ef3fee98579b94964e3bb1cb3e427262c8c068d52319"
    (hex_of (T.blake2b256 "abc"));
  Alcotest.(check string) "blake2b-224 of abc"
    "9bd237b02a29e43bdd6738afa5b53ff0eee178d6210b618e4511aec8"
    (hex_of (T.blake2b224 "abc"));
  Alcotest.(check bool) "the two widths are different functions" false
...
let hash_types () =
  ...
  Alcotest.(check bool) "wrong width refused" true
    (match T.Hash.Tx_id.of_bytes (String.make 28 '\000') with
     | Error _ -> true | Ok _ -> false);
  let h = T.Hash.Addr_key_hash.digest "abc" in
  Alcotest.(check string) "digest agrees with the raw function"
    (hex_of (T.blake2b224 "abc")) (T.Hash.Addr_key_hash.to_hex h);
...
let coin_arithmetic () =
  ...
  Alcotest.(check bool) "above max supply refused" true
    (match T.Coin.of_lovelace 45_000_000_000_000_001L with
     | Error _ -> true | Ok _ -> false);
  Alcotest.(check bool) "max supply itself is fine" true
  ...
  Alcotest.(check bool) "sum past max supply is an error" true
    (match T.Coin.add (c 45_000_000_000_000_000L) (c 1L) with
     | Error _ -> true | Ok _ -> false);
  Alcotest.(check bool) "underflow is an error" true
    (match T.Coin.sub (c 1L) (c 2L) with Error _ -> true | Ok _ -> false);
  Alcotest.(check bool) "mul overflow is an error" true
    (match T.Coin.mul (c 1_000_000_000_000L) 1_000_000 with
     | Error _ -> true | Ok _ -> false);
  Alcotest.(check int64) "sum adds up" 6L
    (T.Coin.to_lovelace (get_coin (T.Coin.sum [ c 1L; c 2L; c 3L ])));
  (match T.Coin.diff (c 1L) (c 3L) with
   | `Neg d -> Alcotest.(check int64) "diff reports sign separately" 2L (T.Coin.to_lovelace d)
...
let coin_decimal () =
  ...
  in
  List.iter round_trip [ "0"; "1"; "1.5"; "0.000001"; "1.234567"; "45000000000" ];
  Alcotest.(check int64) "1 ada is a million lovelace" 1_000_000L
    (T.Coin.to_lovelace (get_coin (T.Coin.of_ada_string "1")));
  Alcotest.(check int64) "partial fraction pads right" 1_200_000L
    (T.Coin.to_lovelace (get_coin (T.Coin.of_ada_string "1.2")));
  Alcotest.(check bool) "seven decimals refused, not rounded" true
    (match T.Coin.of_ada_string "1.2345678" with Error _ -> true | Ok _ -> false);
  List.iter
    (fun s ->
      Alcotest.(check bool) ("refuses " ^ s) true
...
let unsigned_quantities () =
  ...
  Alcotest.(check string) "2^64-1 prints unsigned" "18446744073709551615"
    (T.Quantity.to_string big);
  Alcotest.(check bool) "2^64-1 is greater than one" true
    (T.Quantity.compare big T.Quantity.one > 0);
  Alcotest.(check bool) "signed comparison would have said otherwise" true
    (Int64.compare (T.Quantity.to_int64_unsigned big) 1L < 0);
  Alcotest.(check bool) "2^64-1 does not fit an int" true
    (T.Quantity.to_int big = None);
  Alcotest.(check bool) "addition past 2^64 is an error" true
    (match T.Quantity.add big T.Quantity.one with Error _ -> true | Ok _ -> false);
  Alcotest.(check bool) "zero is not a representable quantity" true
    (match T.Quantity.of_int64_unsigned 0L with Error _ -> true | Ok _ -> false)
...
let multi_asset () =
  ...
  Alcotest.(check bool) "33-byte asset name refused" true
    (match T.Asset_name.of_bytes (String.make 33 'x') with
     | Error _ -> true | Ok _ -> false);
  Alcotest.(check bool) "32-byte asset name allowed" true
    (match T.Asset_name.of_bytes (String.make 32 'x') with
     | Ok _ -> true | Error _ -> false);
  Alcotest.(check string) "empty asset name is legal" "" (T.Asset_name.to_bytes (nm ""));
  ...
  Alcotest.(check string) "repeats combined" "8"
    (T.Quantity.to_string (Option.get (T.Multi_asset.find m a1)));
  let m' = get (T.Multi_asset.of_list [ (a2, q 7); (a1, q 8) ]) in
  Alcotest.(check bool) "order of construction does not matter" true
    (T.Multi_asset.equal m m');
...
let values () =
  ...
  Alcotest.(check bool) "ada-only detected" true
  ...
  Alcotest.(check string) "assets add" "7"
  ...
  Alcotest.(check string) "assets subtract" "3"
  ...
  let all = get (T.Value.sub v1 (T.Value.make (c 0L) (get (T.Multi_asset.of_list [ (a1, q 5) ])))) in
  Alcotest.(check int) "exhausted asset is dropped" 0
    (T.Multi_asset.size all.T.Value.assets);
  Alcotest.(check bool) "shortfall is an error, not a clamp" true
    (match T.Value.sub v2 v1 with Error _ -> true | Ok _ -> false);
  Alcotest.(check bool) "missing asset is an error" true
...
let mint () =
  ...
  Alcotest.(check bool) "zero delta refused" true
    (match T.Mint.of_list [ (a1, 0L) ] with Error _ -> true | Ok _ -> false);
  Alcotest.(check bool) "duplicate asset refused" true
    (match T.Mint.of_list [ (a1, 1L); (a1, 2L) ] with Error _ -> true | Ok _ -> false)
...
let icarus_master () =
  ...
  Alcotest.(check string) "no passphrase"
  ...
  Alcotest.(check string) "with passphrase"
  ...
  Alcotest.(check bool) "entropy under 16 bytes refused" true
    (match K.Icarus.of_entropy (String.make 15 '\000') with
     | Error _ -> true | Ok _ -> false);
  Alcotest.(check bool) "entropy over 32 bytes refused" true
...
let derivation () =
  ...
  Alcotest.(check string) "soft child"
  ...
  Alcotest.(check string) "hardened child"
  ...
  Alcotest.(check string) "public-only soft derivation agrees"
  ...
  Alcotest.(check bool) "hardened from public is refused" true
    (match K.Xpub.derive (K.Xprv.public root) Int32.min_int with
     | Error `Hardened_from_public -> true | _ -> false)
...
let signing () =
  ...
  Alcotest.(check bool) "verifies" true (K.Xpub.verify pub ~signature:sg "hello cardano");
  Alcotest.(check bool) "does not verify a different message" false
    (K.Xpub.verify pub ~signature:sg "goodbye cardano");
  (* Deterministic by RFC 8032 -- no generator anywhere in this library. *)
  Alcotest.(check string) "signing is deterministic" (hex_of sg)
    (hex_of (K.Xprv.sign k "hello cardano"));
  Alcotest.(check int) "key hash is 28 bytes" 28
...
let paths () =
  ...
  Alcotest.(check string) "cip-1852 rendering" "m/1852'/1815'/0'/0/5" (P.to_string p);
  Alcotest.(check string) "shell-safe marker" "m/1852h/1815h/0h/0/5"
    (P.to_string ~marker:`H p);
  let parsed = getp ((P.of_string "m/1852'/1815'/0'/0/5")) in
  Alcotest.(check bool) "parses its own rendering" true (P.equal p parsed);
  Alcotest.(check bool) "accepts the h marker too" true
    (P.equal p (getp ((P.of_string "m/1852h/1815h/0h/0/5"))));
  let stake = getp ((P.stake ~account:0l)) in
  Alcotest.(check string) "stake path" "m/1852'/1815'/0'/2/0" (P.to_string stake);
  (* An index at or above 2^31 without a marker would silently mean hardened. *)
  Alcotest.(check bool) "unmarked hardened index refused" true
    (match P.of_string "m/2147483648" with Error _ -> true | Ok _ -> false);
  List.iter
    (fun s -> Alcotest.(check bool) ("refuses " ^ s) true
