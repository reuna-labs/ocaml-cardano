...
let request_framing () =
  ...
  Alcotest.(check (option string))
    "jsonrpc version" (Some "2.0")
    (match field "jsonrpc" with Some (`String s) -> Some s | _ -> None);
  Alcotest.(check (option string))
    "method name" (Some "queryLedgerState/tip")
  ...
  Alcotest.(check bool)
    "no params field when there are none" true
    (field "params" = None);
  let with_params =
    R.Codec.request ~id:1 (R.Ogmios.query_utxo_by_address [ "addr1x" ])
  in
  Alcotest.(check bool)
    "params present when there are some" true
...
let protocol_parameters_from_ogmios_vector () =
  let body =
    read_file "../conformance/fixtures/ogmios-protocol-parameters.json"
  in
  ...
  Alcotest.(check int64)
    "minFeeConstant, unwrapped from ada.lovelace" 30L pp.P.min_fee_b;
  Alcotest.(check int)
    "maxTransactionSize, unwrapped from bytes" 14_285 pp.P.max_tx_size;
  Alcotest.(check int64)
    "minUtxoDepositCoefficient" 10_780_188L pp.P.coins_per_utxo_byte;
  ...
  (* An execution budget near 2^63. Reading these as anything narrower than
     int64 would wrap. *)
  Alcotest.(check int64)
    "maxExecutionUnits memory" 6_852_459_139_119_258_802L
    pp.P.max_tx_ex_units.P.mem;
  Alcotest.(check int64)
    "maxExecutionUnits cpu" 1_779_215_926_808_906_173L
  ...
  Alcotest.(check string)
    "scriptExecutionPrices.memory stays exact" "1602135494687083999/2500"
    (Format.asprintf "%a" T.Rational.pp pp.P.price_mem);
  Alcotest.(check string)
    "scriptExecutionPrices.cpu stays exact"
  ...
  Alcotest.(check int)
    "reference-script range comes from the node" 25_600 pp.P.ref_script_range;
  Alcotest.(check string)
    "and the multiplier too" "6/5"
...
let float_valued_parameter () =
  let r =
    get
      (Result.map_error
         (fun m -> R.Error.Decode { method_ = "-"; reason = m })
         (T.Rational.of_decimal_string "1.2"))
  in
  Alcotest.(check string)
    "1.2 is exactly 6/5" "6/5"
    (Format.asprintf "%a" T.Rational.pp r);
  let r =
    get
      (Result.map_error
         (fun m -> R.Error.Decode { method_ = "-"; reason = m })
         (T.Rational.of_decimal_string "15"))
  in
  Alcotest.(check string)
    "an integer has denominator one" "15"
    (Format.asprintf "%a" T.Rational.pp r);
  let body =
    read_file "../conformance/fixtures/ogmios-protocol-parameters.json"
  in
  ...
  Alcotest.(check string)
    "the base rate recovers to the digits the node sent"
  ...
  Alcotest.(check bool)
    "and it is the value the node meant" true
    (Float.abs
...
let errors_are_typed () =
  ...
  (match
     R.Codec.response ~id:1 m
       {|{"jsonrpc":"2.0","method":"queryLedgerState/epoch","result":5,"id":2}|}
   with
  ...
  (match
     R.Codec.response ~id:1 m
       {|{"jsonrpc":"2.0","method":"queryLedgerState/tip","result":5,"id":1}|}
   with
  ...
  Alcotest.(check bool)
    "transport failures retry" true
    (R.Error.is_retryable (R.Error.Transport "reset"));
  Alcotest.(check bool)
    "ledger rejections do not" false
    (R.Error.is_retryable
       (R.Error.Rpc { code = 3005; message = "x"; data = None }))
...
let () =
  Alcotest.run "cardano-rpc"
    [
      ...
      ( "ogmios",
        [
          Alcotest.test_case "protocol parameters" `Quick
            protocol_parameters_from_ogmios_vector;
          Alcotest.test_case "float-valued parameter" `Quick
            float_valued_parameter;
        ] );
    ]
